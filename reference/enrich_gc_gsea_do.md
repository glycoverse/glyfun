...
Performs glycan-centric Disease Ontology (DO) Gene Set Enrichment
Analysis (GSEA). It ranks proteins within each glycan trait separately,
then compares enriched disease terms across traits by running GSEA for
each trait.
...
- ont:
  ...
  [`DOSE::gseDO()`](https://rdrr.io/pkg/DOSE/man/gseDO.html). Defaults
  to "HDO".
...
[`clusterProfiler::compareCluster()`](https://rdrr.io/pkg/clusterProfiler/man/compareCluster.html),
[`DOSE::gseDO()`](https://rdrr.io/pkg/DOSE/man/gseDO.html)
