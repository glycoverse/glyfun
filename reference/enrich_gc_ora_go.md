...
Performs glycan-centric Gene Ontology (GO) Over-Representation Analysis
...
functional annotations. It identifies which biological functions are
significantly enriched in proteins exhibiting specific glycosylation
changes, grouping the differential analysis results by trait before
performing ORA.
...
- `raw_result`: The raw clusterProfiler clusterProfResult object The
  list has classes `glyfun_gc_ora_go_res`, `glyfun_gc_ora_res`, and
  `glyfun_res`.
